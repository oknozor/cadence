//! Shazam signature format encoding and decoding.
//!
//! This module handles the binary format used by Shazam's audio fingerprint API.
//! It can encode frequency peaks into a binary signature and decode signatures
//! back into their constituent parts.
...
/// A frequency peak detected in an FFT pass.
pub struct FrequencyPeak {
    /// The FFT pass number when this peak was detected.
    pub fft_pass_number: u32,
    /// The magnitude of the peak.
    pub peak_magnitude: u16,
    /// The corrected frequency bin of the peak.
    pub corrected_peak_frequency_bin: u16,
}

/// Frequency bands used by Shazam for peak detection.
...
pub struct DecodedSignature {
    ...
    /// Map of frequency bands to their detected peaks.
    pub frequency_band_to_sound_peaks: HashMap<FrequencyBand, Vec<FrequencyPeak>>,
}

impl DecodedSignature {
    /// Decode a signature from raw binary data.
    pub fn decode_from_binary(data: &[u8]) -> Result<Self, Box<dyn std::error::Error>> {
    ...
    /// Decode a signature from a data URI.
    ...
    pub fn encode_to_binary(&self) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    /// Encode the signature to a data URI format.
    pub fn encode_to_uri(&self) -> Result<String, Box<dyn std::error::Error>> {
        Ok(format!(
            "{}{}",
            DATA_URI_PREFIX,
